...
//---------------------------------------------------------- experimental ---------------------------------
// my TangleKit classes 
// draggable type of deals

Tangle.classes.TKAdjustableNumberMy = {
    ...
    updateCursor: function () {
        var body = document.getElement("body");
        if (this.isActive()) { 
		this.element.addClass("TKCursorDragHorizontal");
		//body.addClass("TKCursorDragHorizontal");
	}
        else { 
		this.element.removeClass("TKCursorDragHorizontal"); 
    ...
    touchDidMove: function (touches) {
        // allow initial string value for this class - dvrnd
        var checked_value = isNaN( this.valueAtMouseDown ) ? 0 : this.valueAtMouseDown ; 
        var value = checked_value + touches.translation.x / 20 * this.step;
        // one exception
        
        value = ((value / this.step).round() * this.step).limit(this.min, this.max);
        this.tangle.setValue(this.variable, value);
        this.element.getChildren().each( function (child, index) {
            child.style.display = (index != value) ? "none" : "inline";
         });
        this.updateHelp();
    },
